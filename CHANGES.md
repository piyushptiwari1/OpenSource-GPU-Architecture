...
| Fixed-function graphics path: host-driven rasterizer (point/line/rect/triangle) wired into the gpu top; `raster_writer` streams pixels into the data-memory framebuffer as an extra memory-controller consumer, coherent with SIMT loads through the shared L2 | `src/rasterizer.sv` (from PR #55) + `src/raster_writer.sv`, wired in `src/gpu.sv` | `test/test_raster_hw_e2e.py`: hardware triangle+rect vs software reference pixel-for-pixel, then SIMT read-back kernel row sums |
